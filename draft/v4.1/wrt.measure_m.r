...
f.qua = '~/Work/mega/mwceph/lightcurve/20160527/quality/quality.csv'
...
clc.dir = '~/Work/mega/mwceph/lightcurve/20160527/calilcs/'
...
f.tex = '~/Work/mega/mwceph/draft/v4.1/tables/measure_m.txt'
f.hed = '~/Work/mega/mwceph/draft/v4.1/tables/measure_m_h.dat'
...
for (i.dat in 1:n.dat) {
    ...
    for (i in 1:nrow(lc)) {
        ...
        objb = gsub('BETA','BET',objb)
        objb = gsub('L Car','l Car',objb)
        ...
        ts = sprintf('%8s%11s%6s%4s%4s%4s',objb,mjd,mag,err,err.2mass,err.nd4)
