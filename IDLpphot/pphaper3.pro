...
;   f_fit = fits[i_fit]
   f_fco = repstr(f_fit,'.fits','.fco')
   rdfile,f_fco,str_fco,fmt_fco,2,fco,nl_fco
   spawn,'head -1 '+f_fco,fcohead
   meanfwhm = float(strmid(fcohead,10,5))
   meanfwhm = meanfwhm[0]
...
; update x y coordinates before doing photometry
; gaussian kernel used as a psf
   for ixy=0,n_elements(xc)-1 do begin
      xcnew = 0
      ycnew = 0
      starfit,image,xc[ixy],yc[ixy],xcnew,ycnew,FWHM1
      if abs(xc[ixy]-xcnew) lt 2.5 and abs(yc[ixy]-ycnew) lt 1.5 and fwhm1 lt 10 then begin
        ;spawn,'echo '+rmfgspc(xc[ixy]) +' ' + rmfgspc(yc[ixy]) +' '+ rmfgspc(xcnew) +' '+ rmfgspc(ycnew)+' >> '+f_fco+'test_distorsion.dat'
         xc[ixy] = xcnew
         yc[ixy] = ycnew
      endif
   endfor
   ...
   for i=0,n_elements(xc)-1 do begin
      printf,lun,fco(i).id,xc(i),yc(i),mags(*,i),errap(*,i),sky(i),skyerr(i),format=fmt_apr1
   endfor
   cf,lun
   ...
   printf,lun,'   ID       X       Y         M1      M2       M3        M4      M5       M6       M7       M8       M9       M10       M11      M12      E1       E2       E3       E4       E5       E6       E7       E8       E9       E10      E11      E12     SKY     SKYERR'
   for i=0,n_elements(xc)-1 do begin
      printf,lun,fco(i).id,xc(i),yc(i),mags(*,i),errap(*,i),sky(i),skyerr(i),format=fmt_apr2
   endfor
   cf,lun
   ...
   f_fpr = repstr(f_fit,'.fits','.fpr')
   openw,lun,f_fpr,/get_lun
